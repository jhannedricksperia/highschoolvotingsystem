...
// Fetch election data
$stmt = $conn->prepare("SELECT * FROM election WHERE ElectionID=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Manila time for ModifiedDate
    date_default_timezone_set('Asia/Manila');
    $modifiedDate = date('Y-m-d H:i:s');

    $stmt = $conn->prepare("UPDATE election SET ElectionName=?, StartDateTime=?, EndDateTime=?, G8RepNum=?, G9RepNum=?, G10RepNum=?, G11RepNum=?, G12RepNum=?, ModifiedBy=?, ModifiedDate=? WHERE ElectionID=?");
    $stmt->bind_param('sssiiiiisss', $ElectionName, $StartDateTime, $EndDateTime, $G8RepNum, $G9RepNum, $G10RepNum, $G11RepNum, $G12RepNum, $modifiedBy, $modifiedDate, $electionId);
    $stmt->execute();
    $stmt->close();
    $conn->close();
    header('Location: votingsession.php');
...
<body>
    <div class="edit-container">
        ...
        <form method="post">
            <label>Election ID:
                <input type="text" value="<?php echo htmlspecialchars($election['ElectionID']); ?>" disabled>
            </label>
            <label>Election Name:
                <input type="text" name="ElectionName" value="<?php echo htmlspecialchars($election['ElectionName']); ?>" required>
            </label>
            <label>Start Date/Time:
                <input type="datetime-local" name="StartDateTime" value="<?php echo date('Y-m-d\TH:i', strtotime($election['StartDateTime'])); ?>" required>
            </label>
            <label>End Date/Time:
                <input type="datetime-local" name="EndDateTime" value="<?php echo date('Y-m-d\TH:i', strtotime($election['EndDateTime'])); ?>" required>
            </label>
            <label>G8 Rep:
                <input type="number" name="G8RepNum" value="<?php echo htmlspecialchars($election['G8RepNum']); ?>" required>
            </label>
            <label>G9 Rep:
                <input type="number" name="G9RepNum" value="<?php echo htmlspecialchars($election['G9RepNum']); ?>" required>
            </label>
            <label>G10 Rep:
                <input type="number" name="G10RepNum" value="<?php echo htmlspecialchars($election['G10RepNum']); ?>" required>
            </label>
            <label>G11 Rep:
                <input type="number" name="G11RepNum" value="<?php echo htmlspecialchars($election['G11RepNum']); ?>" required>
            </label>
            <label>G12 Rep:
                <input type="number" name="G12RepNum" value="<?php echo htmlspecialchars($election['G12RepNum']); ?>" required>
            </label>
